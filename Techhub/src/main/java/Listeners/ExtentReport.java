    ...
    private void buildTestNodes(IResultMap tests, LogStatus status) throws Exception {
        ...
        if (tests.size() > 0) {
            for (ITestResult result : tests.getAllResults()) {
                //testCaseCount++;
                test1 = extent1.startTest("For  "+((SheetColumnHeader)result.getParameters()[0]).getFirstname()+" details this  test cases ");
               ...
               for (String group : result.getMethod().getGroups()){
                ...
                if (result.getThrowable() != null) {
                  ...
                  //test1.log(LogStatus.FAIL , String.valueOf(MediaEntityBuilder.createScreenCaptureFromPath(System.getProperty("user.dir") + "//test-output//Screenshot//last1.png").build()));
